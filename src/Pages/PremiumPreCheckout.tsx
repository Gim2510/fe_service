...
export function PremiumPreCheckout() {
    ...
    return (
        <main className={`relative min-h-screen overflow-hidden ${isDark ? "bg-[#111110] text-white" : "bg-[#FAF8F4] text-slate-900"}`}>
            ...
            {isDark && (
                <>
                    <div className="absolute -top-60 -left-60 w-[600px] h-[600px] rounded-full blur-[140px] opacity-[0.06] bg-rose-700 pointer-events-none" />
            ...
            <div className="relative max-w-5xl mx-auto px-6 py-32 space-y-24">
                ...
                <section className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
                    <div className="space-y-7">
                        <motion.span
                            {...fadeUp(0)}
                            className={`block text-xs font-semibold uppercase tracking-widest ${isDark ? "text-rose-500" : "text-rose-700"}`}
                        >
                            Abbonamento Premium
                        </motion.span>
                        ...
                        >
                            ...
                            <span className="text-rose-600">report strategico.</span>
                        </motion.h1>
                        ...
                        <motion.div {...fadeUp(0.22)} className="flex items-center gap-4 pt-2">
                            <button
                                ...
                                className="inline-flex items-center gap-2 px-7 py-3 rounded-xl
                                    ...
                                    transition-colors shadow-lg shadow-rose-700/25 hover:-translate-y-0.5 duration-200"
                            >
                        ...
                        <ul className="space-y-3">
                            {[
                                ...
                                <li key={item} className={`flex items-start gap-2 text-sm ${isDark ? "text-slate-400" : "text-slate-600"}`}>
                                    <ArrowRight size={12} className="text-rose-600 mt-0.5 shrink-0" />
                                    {item}
                                </li>
                            ))}
                        </ul>
                        ...
                        <ul className="space-y-3">
                            {FEATURES.map(({ icon, text }) => (
                                <li key={text} className={`flex items-center gap-3 text-sm ${isDark ? "text-slate-300" : "text-slate-700"}`}>
                                    <span className={`shrink-0 ${isDark ? "text-rose-500" : "text-rose-600"}`}>{icon}</span>
                                    {text}
                                </li>
                            ))}
                        </ul>
                ...
                >
                    <div className={`w-full max-w-lg rounded-2xl border p-10 text-center space-y-6 ${card}`}>
                        <div className={`text-xs font-semibold uppercase tracking-widest ${isDark ? "text-rose-500" : "text-rose-700"}`}>
                            Premium
                        </div>
