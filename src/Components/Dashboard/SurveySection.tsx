    ...
    return (
        <motion.div
            ...
            <div className="p-6 space-y-5">
                <p className={`text-[11px] font-mono uppercase tracking-[0.15em] ${isDark ? "text-slate-500" : "text-slate-400"}`}>Performance Survey</p>

                <div className="grid grid-cols-3 gap-4">
                    {stats.map(s => (
                        <div key={s.label}>
                            <p className={`text-[11px] font-mono uppercase tracking-widest mb-1 ${isDark ? "text-slate-600" : "text-slate-400"}`}>{s.label}</p>
                ...
                {dist.length > 0 && (
                    <div className="space-y-1.5 pt-2 border-t border-stone-800/20">
                        <p className={`text-[11px] font-mono uppercase tracking-widest mb-2 ${isDark ? "text-slate-600" : "text-slate-400"}`}>Distribuzione score</p>
                        {dist.map((d: any) => {
                            const pct = Math.round((d.count / maxDist) * 100);
                            return (
                                <div key={d.range} className="flex items-center gap-2">
                                    <span className={`text-[11px] font-mono w-14 text-right ${isDark ? "text-slate-500" : "text-slate-400"}`}>{d.range}</span>
                                    ...
                                    <span className={`text-[11px] font-mono tabular-nums w-6 text-right ${isDark ? "text-slate-500" : "text-slate-400"}`}>{d.count}</span>
                                </div>
                            );
                        })}
                    </div>
                ...
                {catAvgs.length > 0 && (
                    <div className="space-y-1.5 pt-2 border-t border-stone-800/20">
                        <p className={`text-[11px] font-mono uppercase tracking-widest mb-2 ${isDark ? "text-slate-600" : "text-slate-400"}`}>Score medio per categoria</p>
                        {catAvgs.slice(0, 7).map((c: any) => (
                            <div key={c.category} className="flex items-center gap-2">
                                <span className={`text-[11px] font-medium w-24 truncate ${isDark ? "text-slate-400" : "text-slate-600"}`}>{CAT_LABELS[c.category] ?? c.category}</span>
                                <div className="flex-1 h-3 rounded-full overflow-hidden bg-white/5">
                                ...
                                <span className={`text-[11px] font-mono tabular-nums w-9 text-right ${isDark ? "text-slate-500" : "text-slate-400"}`}>{Math.round(c.averagePercentage)}%</span>
                            </div>
                        ))}
                    </div>
                )}
