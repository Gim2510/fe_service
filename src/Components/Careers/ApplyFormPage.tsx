...
export function ApplyFormPage() {
    ...
    const inputClass = `w-full px-4 py-3 rounded-xl border text-sm focus:outline-none transition-colors ${
        isDark
            ...
            : "bg-[#F8FAFB] border-slate-200 text-slate-900 focus:border-emerald-600"
    }`;
                ...
                <form onSubmit={handleSubmit} className="space-y-5">
                    ...
                    <div className="space-y-2">
                        ...
                        <label className={`flex items-center gap-3 px-4 py-3 rounded-xl border cursor-pointer transition-colors ${
                            isDark
                                ? "border-stone-800/30 text-slate-400 hover:border-emerald-700 hover:text-slate-200"
                                : "border-slate-200 text-slate-500 hover:border-emerald-600 hover:text-slate-700"
                        }`}>
                            <Upload size={15} />
