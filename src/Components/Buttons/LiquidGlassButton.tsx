...
export function LiquidGlassButton({children, onClick, to, type = 'button', variant = "default", className = "", color_text = "white", disabled = false}: LiquidGlassButtonProps) {
    const baseStyle =
        `relative ${disabled ? "cursor-not-allowed" : "cursor-pointer"} rounded-2xl text-${color_text} backdrop-blur-xl bg-white/10 border border-white/20 overflow-hidden transition-all duration-300 group`;
    ...
    const content = (
        <>
            <span className={`absolute inset-0 bg-gradient-to-br from-white/30 via-white/10 to-transparent opacity-40 ${disabled ? "" : " group-hover:opacity-70"} transition-opacity`} />
            <span className={`absolute -inset-[2px] rounded-full bg-gradient-to-r from-cyan-400/30 via-red-400/30 to-yellow-400/30 blur-xl opacity-0 ${disabled ? "" : "group-hover:opacity-50"} transition-opacity duration-500`} />
